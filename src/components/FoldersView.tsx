...
import { Folder as FolderIcon, Plus, Loader2, ChevronRight, LayoutGrid, X, FolderPlus, AlertCircle, CheckCircle2, RefreshCw, Trash2 } from 'lucide-react';
...
export function FoldersView({ onSelectCompany }: Props) {
  ...
  const fetchData = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      // 2. Fetch folders strictly filtered by company_id
        ...
        {folders.length === 0 ? (
          <div className="bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200 py-32 text-center">
            ...
            <p className="text-gray-400 font-medium mb-8">
              Create your first folder to begin building magazines.
            </p>
            <button 
              onClick={() => setIsModalOpen(true)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {folders.map((folder) => (
              <button
                ...
                className="group bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-2xl transition-all text-left flex flex-col justify-between min-h-[240px] relative overflow-hidden"
              >
